    ...
    <div>
      <div class="bordes pl-2 pr-2">
        <v-row>
          ...
          <v-col cols="4"><strong>Orden de Ingreso:</strong>
        ...
        <v-row>
          <v-col cols="3"><strong>Sucursal Asignada:</strong> {{Factura.orden_entrada.sucursal.nombre}}</v-col>
          <v-col cols="3"><strong>Total de la Factura: </strong> L. {{Factura.total}}</v-col>
          <v-col cols="3"><strong>Total Abonado: </strong> L. {{Factura.total_abonado}}</v-col>
          <v-col cols="3"><strong>Saldo Pendiente: </strong> L. {{parseFloat(Factura.total) - parseFloat(Factura.total_abonado)}}</v-col>
        </v-row>
      </div>
      <div class="bordes pl-2 pr-2">
        <v-simple-table class="rowsTable">
          <template v-slot:default>
            ...
            <tr v-for="item in Factura.cuerpo_factura_proveedor">
              ...
              <td>{{item.cantidad}}</td>
              <td><strong>L. {{item.sub_total}}</strong></td>
              <td><strong>L. {{item.precio_unitario}}</strong></td>
              <td>{{item.impuesto.nombre}} %</td>
              <td><strong>L. {{item.total}}</strong></td>
            </tr>
            </tbody>
          </template>
        </v-simple-table>
        ...
        <v-simple-table class="rowsTable">
          <template v-slot:default>
            ...
            <tr v-for="item in Factura.pagos_facturas">
              ...
              <td><strong>L. {{item.total_pagado}}</strong></td>
              <td><strong>L. {{item.saldo_pendiente}}</strong></td>
              <td>
