...
<script>
export default {
  name: "pestana_aplicar_anticipo_cliente",
  data(){
    return{
      suc: '',
      sucursal: '',
  ...
  methods:{
    ...
    registrarNota(){
      this.sucursal = this.cajas.find(({caja}) => caja === this.caja);
      if (this.cuenta.venta_financiera === 1)
        this.suc = this.sucursal.suc
      else this.suc = this.cuenta.sucursal_id
    ...
    registrarNotaPagos(){
      ...
      this.$axios.post('contabilidad/anticipos/nota_credito',{
        ...
        this.$store.commit('quitarPestanaKey',17)
      }).catch((error)=>{
