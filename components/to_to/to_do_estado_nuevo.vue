...
<v-card flat>
  <v-container>
    <v-card flat class="d-flex justify-center">
      <v-card width="50%">
        <v-card-text>Los estados son para identificar en que faceta esta la tarea. Por ejemplo: Abierto, Espera,
          Terminado, etc...</v-card-text>
        <v-form ref="FormToDoGrupoNuevo">
          <v-divider></v-divider>
          <v-text-field v-model="GRUPO.nombre" label="Nombre del Estado" counter
...
export default {
  ...
  created() {
    this.$store.commit('todo/asignar_TITULO', 'Creando Nuevo Estado')
  },
  computed:{
    GRUPO:{
      get: function (){
