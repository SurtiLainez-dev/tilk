  ...
  <v-dialog width="95%" v-model="dialogoC">
    <v-card>
      ...
      <v-card flat tile  class="ma-2">
        <v-simple-table  class="ma-5" dense>
          <template>
            ...
            <tr>
              <td>
                <span v-if="noti.grupal === 0">Individual</span>
                <span v-else-if="noti.grupal === 1">Grupal</span>
              </td>
              <td>{{noti.asunto}}</td>
              <td><b-link @click="solicitarClave">Ver documento</b-link></td>
              <td>
                <v-file-input :disabled="noti.estado === 1" accept="application/pdf" dense v-model="documento"></v-file-input>
              </td>
              <td>
                <b-link v-if="file.file" @click="verDocumento">Ver documento cargado</b-link>
                <span v-else>No se ha cargado documento</span>
              </td>
            </tr>
            </tbody>
          </template>
      ...
      <v-card-actions class="d-flex justify-end">
        <v-btn color="warning" small class="ma-2" dark text tile @click="dialogoC = false">Cerrar</v-btn>
  ...
  methods:{
    abrirDialogo(data){
      ...
      documentos.forEach((item, i)=>{
        if (data.grupal === 0 && parseInt(item.key) === parseInt(this.$store.state.usuario_id)) {
          this.cont = i;
          this.file = item;
        }else if(data.grupal === 1){
          this.cont = 0;
          this.file = documentos[0]
        }
      })
    },
