<template>
  <v-card flat>
    <v-toolbar flat color="grey lighten-3">
      <v-card-title>Ordenes de Entrega Genéricas</v-card-title>
    </v-toolbar>
    <v-row>
      <v-col cols="4">
        <v-btn class="ma-2" tile small color="indigo" dark @click="$router.replace({path:'/inicio/otras_gestiones/nueva_orden_generica'})">Crear Nueva Orden</v-btn>
      </v-col>
      <v-col cols="5"></v-col>
      <v-col cols="3">
    ...
    <v-data-table dense :items="ORDENES" :headers="header" @click:row="selesctOrden"
                  :loading="LOAD_ORDENES" loading-text="Cargando datos ...">
...
export default {
  name: "ordenes_genericas",
  created() {
    this.$store.commit('guardarTitulo', 'Inicio > Otras Gestiones > Ordenes de Entrega Genéricas');
  ...
  computed:{
    ORDENES(){
      return this.$store.state.orden_genericas.ORDENES;
    },
    LOAD_ORDENES(){
